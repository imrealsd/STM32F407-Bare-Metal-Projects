...
{   
    ...
    for (int16_t i = number; i >= 0; i--){
        ...
        for (int16_t j = 0; j < DIGIT_DURATION; j++){
...
void show_time (int8_t hour, int8_t min)
{
    int8_t arr[10] = {0x3f, 0x06, 0x5B, 0x4F, 0X66, 0x6D, 0x7D, 0x07, 0x7F, 0x67};      // 0 to 9 without dot
    int8_t arr_dot[10] = {0xBf, 0x86, 0xDB, 0xCF, 0XE6, 0xED, 0xFD, 0x87, 0xFF, 0xE7};  // 0 to 9 with dot
    int8_t d1, d2, d3, d4;
